...
Stat:SetScript("OnMouseDown", function()
	...
	local dest = (GetNumRaidMembers() > 0 and "RAID") or (GetNumPartyMembers() > 0 and "PARTY") or (IsInGuild() and "GUILD") or ("SAY")
	if UnitLevel("player") == MAX_PLAYER_LEVEL and GetWatchedFactionInfo() then 
		local name, rank, min, max, value = GetWatchedFactionInfo()
		SendChatMessage("I'm currently "..reaction[rank][2].." with "..name.." "..(value-min).."/"..(max-min).." ("..floor((((value-min)/(max-min))*100)).."%).",dest)
	else
		local XP, maxXP = UnitXP("player"), UnitXPMax("player")
		SendChatMessage("I'm currently at "..T.CommaValue(XP).."/"..T.CommaValue(maxXP).." ("..floor((XP/maxXP)*100).."%) experience.",dest)
	end
end)