...
-- T.databars[barNum]:SetHeight(pHeight*3+10)
if C.databars.settings.vertical then
	...
	T.databars[barNum]:SetWidth((numWatched()*pWidth) + ((numWatched()-1)*C.databars.settings.spacing))
end
...
for i = 1, 3 do
	...
	if i == 1 then
		if C.databars.settings.vertical then
			...
			currencybars[i]:CreatePanel("ThickTransparent", pWidth-pHeight-C.databars.settings.spacing, pHeight, "TOPLEFT", T.databars[barNum], "TOPLEFT", pHeight+C.databars.settings.spacing, 0)
		end
	else
		if C.databars.settings.vertical then
			...
			currencybars[i]:CreatePanel("ThickTransparent", pWidth-pHeight-C.databars.settings.spacing, pHeight, "LEFT", currencybars[i-1], "RIGHT", C.databars.settings.spacing + pHeight + C.databars.settings.spacing, 0)
		end
	end
	currencybars[i]["bar"] = {}
	...
	currencybars[i]["bar"]:SetStatusBarColor(0.3,0.3,0.3)
	currencybars[i]["bar"]:SetFrameLevel(2)
	
	
	currencybars[i]["text"] = currencybars[i]["bar"]:CreateFontString("DataBar"..barNum.."_Text"..i, "OVERLAY")
	currencybars[i]["text"]:SetFont(C.media.pixelfont, C["datatext"].fontsize)
	currencybars[i]["text"]:SetPoint("TOPRIGHT", currencybars[i], "TOPRIGHT", -2, -2)
	currencybars[i]["text"]:SetPoint("BOTTOMLEFT", currencybars[i], "BOTTOMLEFT", 2, 2)
...
Stat:SetScript("OnEnter", function()
	if not InCombatLockdown() then
		...
		if C.databars.settings.vertical then
			...
			GameTooltip:AddLine(strtrim(names))
		end
		GameTooltip:Show()
	end
end)
