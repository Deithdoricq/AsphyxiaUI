...
for i = 1, T.maxDatabars do
	...
	if i == 1 then
		...
		if C.databars.settings.vertical then
			...
			T.databars[i]:CreatePanel("ThickTransparent", pWidth, pHeight, "TOPLEFT", T.databars[i-1], "TOPRIGHT", C.databars.settings.spacing, 0)
		end
	end
	...
	T.databars[i].statusbar:SetStatusBarColor(0.3,0.3,0.3)
	...
	T.databars[i].text:SetFont(C.media.pixelfont, C["datatext"].fontsize)
...
local function hideDatabars(self)
	...
	self.text:SetText(T.StatColor.."open")
	self:ClearAllPoints()
...
local function showDatabars(self)
	...
	self.text:SetText(T.StatColor.."close")
	self:ClearAllPoints()
...
T.databars["toggle"].text:SetFont(C.media.pixelfont, C["datatext"].fontsize)
T.databars["toggle"].text:SetJustifyH("LEFT")
T.databars["toggle"].text:SetPoint("CENTER")
T.databars["toggle"].text:SetText(T.StatColor.."close")

if C.databars.settings.vertical then
	T.databars["toggle"]:CreatePanel("ThickTransparent", pWidth, C.databars.settings.height, "TOP", T.databars[T.maxDatabars], "BOTTOM", 0, -C.databars.settings.spacing)
else
	T.databars["toggle"]:CreatePanel("ThickTransparent", pWidth, C.databars.settings.height, "LEFT", T.databars[T.maxDatabars], "RIGHT", C.databars.settings.spacing, 0)
end
